...
// CreateFrame builds a frame for the connection with the provided tcp
// overriding defaults and the additionalLayers added after the TCP header.
...
func NewUDPIPv4(t *testing.T, dut DUT, outgoingUDP, incomingUDP UDP) UDPIPv4 {
	lMAC, err := tcpip.ParseMACAddress(*localMAC)
	if err != nil {
		t.Fatalf("can't parse localMAC %q: %s", *localMAC, err)
	}
...
func (conn *UDPIPv4) SendFrame(frame Layers) {
	outBytes, err := frame.toBytes()
	fmt.Printf("layersToSend after  %v", outBytes)
	if err != nil {
